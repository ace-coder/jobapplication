$(document).ready(function () {
    ...
    $('#skill').on('click', '.add-btn', function () {
        var html = "";
        var i = $('.skill').size() + 1;
        html += '<div class="skill"><h5 class="">Skill Info ' + i + '</h5><div class="form-group"><label class="col-md-4 control-label">Skill Name<span>*</span></label><div class="col-md-8"><input  type="text" name="skillname[]" class="form-control"  value="" placeholder="Skill Name" required/></div></div><div class="form-group"><label class="col-md-4 control-label">Year Of Experience<span>*</span></label><div class="col-md-8"><select id="skillexp_' + i + '" name="skillexp[]" class="form-control" required></select></div></div><div class="remove-btn"><i class="glyphicon glyphicon-minus " ></i></div></div>';

        $('#skill').append(html);
        $('#skillexp_' + i).html($('#skillexp_1').html());
    });
